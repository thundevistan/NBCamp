...
class SignUpActivity : AppCompatActivity() {
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		btn_Single.setOnClickListener {     // 회원가입 버튼 클릭
			...
			var isName = strName.isEmpty()          // 이름 텍스트가 비어있는지 확인, 초기값 : true
			var isId = strId.isEmpty()              // 아이디 텍스트가 비어있는지 확인, 초기값 : true
			var isPassword = strPassword.isEmpty()  // 비밀번호 텍스트가 비어있는지 확인, 초기값 : true
